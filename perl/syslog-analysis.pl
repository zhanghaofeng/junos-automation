...
Example:
  $0 -d -r router.txt -S -w /phc/		Load router list from file router.txt and login router directly
  $0 -p -r router.txt -S -w /phc/		Load router list from file router.txt and login router through proxy server
  $0 -p -q -t proxy -s router -w /phc/	Telnet to proxy and SSH to router
  $0 -p -q -t router -s proxy -w /phc/	SSH to proxy and Telnet to router
  $0 -p -q -T -w /phc/					Telnet to proxy and router
  $0 -p -q -S -w /phc/					SSH to proxy and router
