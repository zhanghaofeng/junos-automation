...
#Others parameters
$fabric_drop_threshold = 10000000;
...
foreach $router (@router) {
	...
	if ( 1 ) {
		if ($opt{d}) {
			...
			print "Previous value, $current_value; Next value, $next_value.\n";
...
`grep _CXP *alarm | sed 's/-alarm:/\t/g' >> $work_path/TODAY_ALARMS`;

check_route();

# send wechat message to mobile. if any alarm reported in current alarm check
`python /asphc/wechat.py $work_path/TODAY_ALARMS "Haofeng|JohnYe|Owen_Yang|Tim|kseptem|wilson|zhaoping"`;
