...
chomp($year = `date '+%Y'`);
chomp($today = `date -d "-1 hour" '+%b %e %H'`);
...
`echo "\n\n** TXP3D Optical report **" >> $work_path/TODAY_ALARMS`;
`grep _CXP *alarm | sed 's/:/\t/g' >> $work_path/TODAY_ALARMS`;
`python /asphc/wechat-gdcmcc.py $work_path/TODAY_ALARMS "allen"`;
...
#		"show route summary | no-more",
		"show log messages | match $year | match \"$today\" | except mgd | no-more",
		"show log messages.0.gz | match $year | match \"$today\" | except mgd | no-more"
		);
	} else {
		@cmd = (
#		"show chassis alarms | no-more",
#		"show route summary | no-more",
		"show log messages | match \"$today\" | except mgd | no-more",
		"show log messages.0.gz | match \"$today\" | except mgd | no-more"
#                "show log messages | match \"$today\" | no-more",
#                "show log messages.0.gz | match \"$today\" | no-more"
		);	
	}
	return @cmd;
}
...
sub getstats
{
	...
	$session->send("show interfaces detail | match \"physical|input|output\" | no-more\n");
	$session->expect( $timeout , "-re" , $rtr_prompt );
	$session->send("\n");
	$session->expect( $timeout , "-re" , $rtr_prompt );
	return;
